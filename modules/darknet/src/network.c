...
{
    int batchSize = net.batch;
    int n = d.X.rows / batchSize; // subdivisions
    //assert(n==net.subdivisions);
    const int pixelsPerImage = d.X.cols; //w*h*3
    float *X = calloc(batchSize*pixelsPerImage, sizeof(float));
    float *y = calloc(batchSize*d.y.cols, sizeof(float));

    int i;
    float sum = 0;
    for(i = 0; i < n; ++i){
        get_next_batch(d, batchSize, i*batchSize, X, y);
        float err = train_network_datum(net, X, y);
        sum += err;
    }
    free(X);
    free(y);
    return (float)sum/(n*batchSize);
}
